...
# Merge all cohorts
cohorts.list <- list(sleep_alspac_birth.data, sleep_alspac_school.data)
...
n_birth <- max(sleep.data$n_alspac_birth, na.rm = T) 
cpg_birth <- sleep.data$probeID[!is.na(n_birth)]

n_school <- max(sleep.data$n_alspac_school, na.rm = T) 
cpg_school <- sleep.data$probeID[!is.na(n_school)]
...
save(meta_results.data, file = "results/meta_results_sleep_alspac.Rdata")
